...
def declare_actions(
    ...
    set_slam = SetLaunchConfiguration('slam', 'True')
    ...
    # SLAM + Navigation unified bringup via Nav2
    ...
    # Remap /odom from mobile_base_controller and /scan
    nav2_group = GroupAction([
        SetRemap(src='/odom', dst='/mobile_base_controller/odom'),
        SetRemap(src='/scan', dst='/scan_raw'),
        nav2_bringup
    ])
    launch_description.add_action(nav2_group)
    ...
    #     "slam_launch.py"
    # ],)
    # slam_bringup_launch = GroupAction([
    #     SetRemap(src='/odom', dst='/mobile_base_controller/odom'),
    ...
    #         FindPackageShare('hunav_gazebo_wrapper'),
    #         'pmb2_params','pmb2_nav_public_sim.yaml'
    #         ]),
    #     {'use_sim_time': True}
    #     ],
    #     remappings=[
    ...
    rviz_bringup_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            "rviz_config": os.path.join(
                pmb2_2dnav, "config", "rviz", "slam.rviz" #navigation.rviz
            ),
            "use_sim_time": "True",
        }.items()
    )
